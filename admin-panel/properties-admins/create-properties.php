...
<?php 
  ...
  if(isset($_POST['submit'])) {
    ...
    OR empty($_POST['description']) OR empty($_POST['price_sqft'])) {
      ...
      $insert = $pdo->prepare("INSERT INTO properties (name, locations, price, beds, baths,
      sq_ft, home_type, year_built, types, description, price_sqft, admin_name, image) 
      ...
      if(move_uploaded_file($_FILES['thumbnail']['tmp_name'], $dir)) {
        ...
        foreach ($_FILES['image']['tmp_name'] as $key => $value) {
          ...
          $insertqry=$pdo->prepare("INSERT INTO `related_images` (`image`, `props_id`) VALUES (:image, :props_id)");
          $insertqry->execute([
            ':image' => $finalimg,
            ':props_id' => $id
          ]);
